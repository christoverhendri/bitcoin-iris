...
SOURCE_FILE = "btc_master_3.csv"  # ganti ke btc_master_3.csv untuk dataset kedua
...
def walk_forward_validation_quantile(df, feature_cols, target_col, n_splits):
    ...
    for i in range(1, n_splits + 1):
        ...
        # Regularisasi ADAPTIF -- root cause ditemukan lewat diagnostic:
        # dengan reg_lambda=2.0 + min_child_weight=10 + quantile ekstrem (Q10/Q90)
        # + data kecil (~76-84 baris), model gagal membuat split SAMA SEKALI
        # (feature_importances_ semua 0 -- model cuma prediksi angka konstan,
        # itu kenapa prediksi identik persis di semua step untuk fold kecil).
        # Solusi: skala regularisasi mengikuti ukuran training set.
        n_train = len(X_train)
        if n_train < 150:
            adaptive_min_child_weight = 1
            adaptive_reg_lambda = 0.5
            adaptive_reg_alpha = 0.0
        else:
            adaptive_min_child_weight = max(2, min(10, n_train // 100))
            adaptive_reg_lambda = 2.0
